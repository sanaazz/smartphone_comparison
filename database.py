...
class internal(Base):
    ...
    id = Column(Integer, primary_key=True,autoincrement=True)
    RAM_GB = Column(String(255))
    Internal_Storage_GB = Column(String(255))

class chipset(Base):
    __tablename__ = 'chipset'
    id = Column(Integer, primary_key=True,autoincrement=True)
    Platform_Chipset = Column(String(510))

class card(Base):
    __tablename__ = 'card'
    id = Column(Integer, primary_key=True,autoincrement=True)
    Card_Slot_Type = Column(String(255))
...
class AddToTable:
    ...
    def addDeviceName(self):
        ...
        device_name.to_sql('device_name', con=self.connection, if_exists='append', index=False)#Platform_Chipset
        
    def addPlatformChipset(self):
        csvDevice = pd.read_csv('./processed_data.csv')
        chipset = csvDevice[['Platform_Chipset']]
        chipset.to_sql('chipset', con=self.connection, if_exists='append', index=False)

    def addInternal(self):
        csvDevice = pd.read_csv('./processed_data.csv')
        Internal = csvDevice[['RAM_GB','Internal_Storage_GB']]
    ...
    def addWLAN(self):
        csvDevice = pd.read_csv('./processed_data.csv')
        wlan = csvDevice[['Comms_WLAN']]
        wlan.to_sql('wlan', con=self.connection, if_exists='append', index=False)
        
    def addBluetooth(self):
        csvDevice = pd.read_csv('./processed_data.csv')
        Bluetooth = csvDevice[['Comms_Bluetooth']]
        Bluetooth.to_sql('bluetooth', con=self.connection, if_exists='append', index=False)#Comms_USB
        
        
    def addUsb(self):
        csvDevice = pd.read_csv('./processed_data.csv')
        usb = csvDevice[['Comms_USB']]
        usb.to_sql('usb', con=self.connection, if_exists='append', index=False)#Comms_USB
